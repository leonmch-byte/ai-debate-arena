...
export async function startServer({ port = 3100, dbPath = 'db/arena.db', simulateFail = [], betaMode = null, log = (...a) => console.log(...a) } = {}) {
  ...
  const scenario = new ScenarioEngine(store, realAdapter, log);
  ...
  async function api(req, res, url) {
    ...
    try {
      ...
      if ((m = url.pathname.match(/^\/api\/scenario\/(ord_\w+)\/turn$/)) && req.method === 'POST') {
        const u = sessionUser(req);
        const isLocal = ['127.0.0.1','::1','::ffff:127.0.0.1'].includes(req.socket.remoteAddress ?? '');
        let identity = u;
        if (!identity && isLocal) identity = { user_id: 'local-dev' };
        if (!identity) return send(401, { error: 'LOGIN_REQUIRED' });
        try {
          const brief = await scenario.userTurn(m[1], identity.user_id, body.text ?? '');
          const obs = await scenario.situationBriefing(m[1]).catch(() => null);
          return send(200, { round: brief.round, reactions: brief.reactions, briefing: obs });
          return r.ok ? send(200, r) : send(502, { error: r.error?.reason_code ?? 'SCENE_ERROR' });
        } catch (e) { return send(400, { error: e.code ?? 'SCENE_ERROR', message: e.message }); }
      }
      if (req.method === 'POST' && url.pathname === '/api/scenario/start') {
        ...
        let identity = u;
        if (!identity && isLocal) identity = { user_id: 'local-dev', email: 'local@server' };
        try {
          const st = scenario.start(identity.user_id, { problem: body.problem ?? '', days: body.days ?? '3' });
          if (body.situation?.politics_situation) {
            const OP = {
              credit: '散会了。走廊上，那个在汇报里"代表作"了方案的同事追上来，笑着说：别多想哈，领导就吃我这套表达。你笑了笑没接话。电梯门开了——领导恰好也在。他看着你：\u300c对了，刚才那个方案，听说前期你花了不少功夫？\u300d',
              edge: '又一次重要会议没有通知你。散会后茶水间，那位司龄最长的老员工看到你，欲言又止，最后只说了句：\u300c年轻人，别把所有力气都用在活儿上。\u300d',
              scapegoat: '项目出了问题，领导在群里@全员要求复盘。下班前，那位同事单独找到你：\u300c上面在找人背这个，咱俩关系好我才提醒你——你最好想想怎么说。\u300d'
            };
            const key = body.situation.politics_situation;
            const custom = OP[key] ? { actor: 'boss', text: OP[key] } : null;
            st.opening = custom ?? st.opening;
          }
          return send(200, st);
        } catch (e) { return send(400, { error: e.code ?? 'SCENE_ERROR', message: e.message }); }
      }
      ...
      if ((m = url.pathname.match(/^\/api\/orders\/(ord_\w+)\/run$/)) && req.method === 'POST') {
        const h0 = store.getOrder(m[1]);
        if (!h0.some(e => e.type === 'PAYMENT_SUCCEEDED'))
          return send(400, { error: 'PAYMENT_REQUIRED', message: '未支付订单不能启动履约（§3.1 T1）' });
        if (body.topic) topicsByOrder[m[1]] = body.topic;
