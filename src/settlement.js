...
// REPLACED 前驱现金经 §2.5 结转进入后继 funding，自身贡献 0。
export function verifyE1(events) {
  ...
  for (const e of events) {
    switch (e.type) {
      ...
      case 'SURCHARGE_EXECUTED':   paid += e.amount_cents ?? 0; break;
      ...
      case 'ORDER_CONFIRMED':
        ...
        break;
      case 'ITEM_LOCKED':
      case 'SURCHARGE_EXECUTED_B': // 不存在，防呆占位
        break;
      default: break;
    }
    if (e.type === 'ITEM_LOCKED' && e.data?.funding_split)
      fundingByItem.set(e.item_id, e.data.funding_split);
    if (e.type === 'SURCHARGE_EXECUTED' && e.data?.funding_split)
      fundingByItem.set(e.data.successor_item_id, e.data.funding_split);
  }
  const { projectItems } = require === undefined
    ? { projectItems: null } : { projectItems: null };
  return { events, paid_cash_cents: paid, refunded_cash_cents: refunded, fundingByItem };
}

// 独立完整版（避免循环依赖，items 投影由调用方传入或内部导入）
...
export function E1(events) {
  ...
  for (const e of events) {
    ...
    else if (e.type === 'SURCHARGE_EXECUTED') paid += e.amount_cents ?? 0;
    else if (e.type === 'REFUND_EXECUTED') refunded += e.amount_cents ?? 0;
    else if (e.type === 'ORDER_CONFIRMED')
      for (const it of (e.data.items ?? []))
        if (it.funding_split) fundingByItem.set(it.item_id, it.funding_split);
    else if (e.type === 'ITEM_LOCKED' && e.data?.funding_split)
      fundingByItem.set(e.item_id, e.data.funding_split);
    else if (e.type === 'SURCHARGE_EXECUTED' && e.data?.funding_split)
      fundingByItem.set(e.data.successor_item_id, e.data.funding_split);
  }
  let expected = 0;
