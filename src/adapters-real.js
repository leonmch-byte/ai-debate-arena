...
// Key 来源：~/.arena_env（或 ARENA_ENV_FILE 指定），仅服务器本地，不入库不入 git。
// 就绪判定按提供商：火山(ark)=ARK_API_KEY+ARK_ENDPOINT；阿里(dashscope)=DASHSCOPE_API_KEY。互相独立，谁配好谁可用。
...
export class RealAdapter {
  ...
  #ready(provider) {
    if (provider === 'ark') return !!(process.env.ARK_API_KEY && process.env.ARK_ENDPOINT);
  ...
  async run(model_id, input = {}) {
    ...
    const key = isArk ? process.env.ARK_API_KEY : process.env.DASHSCOPE_API_KEY;
