...
test('M4-1 正常履约：LOCKED→STARTED→COMPLETED，结果引用落账（T1/T3/T5）', async () => {
  ...
  const r = await fulfillItem(store, orderId, items[0].item_id, 'gpt-4o',
    new ScriptedAdapter({ 'gpt-4o': [{ ok: 'res_report_1' }] }), { backoffMs: 0 });
...
test('M4-2 A类耗尽：3次恢复（退避1/2/4）→EXHAUSTED→FAILED_FINAL（T4/T6）', async () => {
  ...
  const adapter = new ScriptedAdapter({ 'qwen-max': [{ fail: 'MODEL_RATE_LIMITED' }] });
  const r = await fulfillItem(store, orderId, items[0].item_id, 'qwen-max', adapter, { backoffMs: 0 });
...
test('M4-3 A类恢复成功：1次恢复后 COMPLETED，用户无感（§4.2）', async () => {
  ...
  const adapter = new ScriptedAdapter({ 'kimi': [{ fail: 'MODEL_TIMEOUT' }, { ok: 'res_after_retry' }] });
  const r = await fulfillItem(store, orderId, items[0].item_id, 'kimi', adapter, { backoffMs: 0 });
  assert.equal(r.completed, true);
...
test('M4-4 B类不进恢复环：直接 FAILED_FINAL（§4.1）', async () => {
  const { store, dir, orderId, items } = await paidOrder(['deepseek-v3']);
  const adapter = new ScriptedAdapter({ 'deepseek-v3': [{ fail: 'MODEL_AUTH_FAILURE' }] });
  const r = await fulfillItem(store, orderId, items[0].item_id, 'deepseek-v3', adapter, { backoffMs: 0 });
  assert.equal(r.completed, false);
...
test('M4-5 C类：修正1次成功 / 修正后仍失败→终判（§4.2）', async () => {
  {
    const { store, dir, orderId, items } = await paidOrder(['glm-4-plus']);
    const adapter = new ScriptedAdapter({ 'glm-4-plus': [{ fail: 'MODEL_RESPONSE_INVALID' }, { ok: 'res_corrected' }] });
    const r = await fulfillItem(store, orderId, items[0].item_id, 'glm-4-plus', adapter, { backoffMs: 0 });
    assert.equal(r.completed, true);
    const h = store.getOrder(orderId);
...
test('M4-6 T1前置：未支付订单直接 ITEM_LOCKED 被拒（§3.1 T1）', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'arena-m4-'));
  const store = new EventStore(join(dir, 't.db'));
  const q = createQuote(store, { user_id: 'u_m4', model_ids: ['kimi'] });
  assert.throws(
...
test('M4-7 reason_code 全集分类正确 + 非法code被拒 + UPSTREAM_SUSPENDED全链路（§1.5）', async () => {
  ...
  const { store, dir, orderId, items } = await paidOrder(['qwen-max', 'deepseek-v3']);
  const adapter = new ScriptedAdapter({ 'qwen-max': [{ fail: 'UPSTREAM_SUSPENDED' }] });
  await fulfillItem(store, orderId, items[0].item_id, 'qwen-max', adapter, { backoffMs: 0 });
  const h = store.getOrder(orderId);
